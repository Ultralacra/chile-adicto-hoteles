...
// Input WP export path (prefer the file in lib with the latest date in the filename)
const libDir = path.resolve('./lib')
const outFile = path.resolve('./lib/arquitectura.json')
...
function findWpExportFile() {
  ...
  // pick the last lexicographically (usually includes timestamp)
  candidates.sort()
...
function stripHtmlPreserveText(html) {
  if (!html) return ''
  // remove styles and scripts and svgs heavy markup
  let s = String(html)
  ...
  // convert anchors to their text content (fallback to href if no text)
...
function splitParagraphsFromHtml(html) {
  if (!html) return []
  // try to split by <p> to preserve content blocks
  const ps = []
  ...
  while ((m = pRegex.exec(html)) !== null) {
    ...
    // ignore non-p content before next <p>
    const pHtml = m[0]
    const text = stripHtmlPreserveText(pHtml)
    if (text) ps.push(text)
  }
  // trailing content without <p>
  const tail = html.slice(lastIndex)
  const tailText = stripHtmlPreserveText(tail)
  if (tailText) {
    // split on double newlines to keep logical blocks
...
function parseTitle(raw) {
  ...
  let s = String(raw)
  // Normalize <br> to newline and strip tags for clean split
  s = s.replace(/<br\s*\/?>(\s*)/gi, '\n')
  // Keep subtitle content inside span if present
...
function extractImages(post) {
  ...
  // also anchors that link to images (common in galleries)
  const aImgRegex = /<a[^>]+href=["']([^"']+\.(?:png|jpe?g|webp|gif))(?:\?[^"']*)?["']/gi
  while ((m = aImgRegex.exec(content)) !== null) {
    urls.add(m[1])
  }
...
function buildItems(wp) {
  ...
  for (const p of posts) {
    ...
    // Use Spanish as the anchor to avoid duplicates
    if (lang !== 'es') continue
    ...
    if (enPost) {
      for (const u of extractImages(enPost)) images.includes(u) || images.push(u)
    }

    // categories from taxonomy (take English slugs/names if available via p.taxonomies.category)
    ...
    // keep only Architecture posts (both ES/EN names)
    const isArchitecture = categories.some(x => /^(ARQUITECTURA|ARCHITECTURE)$/i.test(x))
...
function main() {
  ...
  console.log(`Importados ${items.length} items desde ${path.basename(inFile)} a ${path.basename(outFile)} (contenido íntegro preservado).`)
}
