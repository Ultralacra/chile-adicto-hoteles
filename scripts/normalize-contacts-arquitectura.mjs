...
function normalizeUrl(u) {
  ...
  if (href) s = href[1]
  s = s.replace(/^["'\s]+|["'\s]+$/g, '')
  // add protocol for domains or www
  if (/^www\./i.test(s) || /^[a-z0-9\-]+\.[a-z]{2,}/i.test(s)) {
    if (!/^https?:\/\//i.test(s)) s = 'https://' + s
  }
  // ensure https for instagram
...
function extractContacts(line) {
  ...
  // website & instagram from href or text
  ...
  if (!pm && isAddressLine(text)) {
    let addr = text
    // If the line also contains photo credit labels, cut them off
  ...
  for (const item of arr) {
    ...
    for (const lang of ['es','en']) {
      ...
      for (const line of d) {
        ...
        // Preservar SIEMPRE el contenido original
        newDesc.push(line)
      }
    ...
    if (instagram) {
      ...
      try {
        ...
        if (iu.hostname.replace(/^www\./,'').toLowerCase() === 'instagram.com' && !p) {
          instagram = ''
        }
      } catch {}
    }
    ...
    // ensure categories
    item.categories = item.categories && item.categories.length ? item.categories : ['ALL','ARCHITECTURE']
  }

  fs.writeFileSync(file, JSON.stringify(arr, null, 2), 'utf8')
  console.log('Normalización de contactos completa para', arr.length, 'items. Contenido preservado; se extrajeron website/instagram/email/phone/photosCredit/address y se recalcularon displays.')
}
